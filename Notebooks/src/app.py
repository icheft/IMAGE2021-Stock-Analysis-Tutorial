...
def app(stock_id='0050.TW'):
    st.write('# Python 投資分析')
    stock_id = stock_id
    stock_obj = yf.Ticker(stock_id)

    # 印出我們的標題

    st.write(f'## 對 {stock_obj.info["longName"]} 進行分析')
    ...
    st.write(f'### 日成長變化 (本金為 {init_balance} 元)')
    ...
    st.write(f'''+ 如果當初投資 {init_balance} 元，經過 {year_difference} 年後，變成 {total_balance[-1]} 元
+ 年化報酬率為 {cgar * 100}%
+ 成長率為 {return_rate * 100}%''')
...
def app_px(stock_id='0050.TW'):
    ...
    st.write('### 股市回測線：')
    #  st.line_chart(stock_df['Adj Close'])
    fig = px.line(stock_df['Adj Close'], labels={
...
#     st.bar_chart(stock_yearly_returns.dropna())
    fig = px.bar(beautify_syr, x='Date', y='Percentage Change', hover_data=['Percentage Change', 'Date'], color='Percentage Change')
    st.plotly_chart(fig, use_container_width=False)
    ...
    st.write(f'### 日成長變化 (本金為 {init_balance} 元)')
    fig = px.line(total_balance, labels={
...
# # 多一點變化：
# def main():
#     st.sidebar.title('使用者資料輸入')
#     stock_id = st.sidebar.text_input('請輸入想分析的股票代號', '0050.TW')
    
#     app(stock_id)
    
#     st.write('''***
# 大家可以自行參考 [Streamlit 官網](https://docs.streamlit.io/en/stable/api.html#display-interactive-widgets) 的 Display Interactive Widgets 部分，加上「選擇日期」或是其他功能。''')
